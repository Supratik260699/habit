...
router.post('/user-view', (req, res) => {
    User.findOne({
        ...
        .then(users => {
            users.view = users.view === 'daily' ? 'weekly' : 'daily';
...
router.post('/dashboard', (req, res) => {
    ...
    Habit.findOne({ content: content, email: email }).then(habit => {
        if (habit) {
            ...
            //Updating the already exists habit
            ...
            dates.find(function (item, index) {
                if (item.date === today) {
                    ...
                    req.flash(
                        'error_msg',
                        'This habit was already exists in the list!'
                    );
                    res.redirect('back');
                }
                else {
        ...
        .then(habit => {
            ...
            habit.save()
                .then(habit => {
                    req.flash(
                        'success_msg',
                        habit.favorite ? 'A New habit added to Favorites!' : 'Habit removed from Favorites!'
                    );
                    return res.redirect('back');
                })
                .catch(err => console.log(err));
